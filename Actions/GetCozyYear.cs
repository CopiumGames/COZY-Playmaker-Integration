/*
 * Summary:
 * This script gets the current year from COZY: Stylized Weather 3 and stores it in a PlayMaker variable.
 *
 * License:
...
[HutongGames.PlayMaker.Tooltip("Gets the current year from COZY: Stylized Weather 3")]
public class GetCozyYear : FsmStateAction
{
    [RequiredField]
    [UIHint(UIHint.Variable)]
    [HutongGames.PlayMaker.Tooltip("Store the current year")]
    ...
    void DoGetYear()
    {
        ...
        if (cozyWeather == null)
        {
            Debug.LogWarning("GetCozyYear: CozyWeather instance is null.");
            storeValue.Value = 0f;
            return;
        }
        ...
        if (timeModule == null)
        {
            Debug.LogWarning("GetCozyYear: TimeModule is null.");
            storeValue.Value = 0f;
            return;
        }

        storeValue.Value = timeModule.currentYear;
    }
}
