/*
 * Summary:
 * This script gets the current minute from COZY: Stylized Weather 3 and stores it in a PlayMaker variable.
 *
 * License:
...
[HutongGames.PlayMaker.Tooltip("Gets the current minute from COZY: Stylized Weather 3")]
public class GetCozyMinute : FsmStateAction
{
    [RequiredField]
    [UIHint(UIHint.Variable)]
    [HutongGames.PlayMaker.Tooltip("Store the current minute")]
    ...
    void DoGetMinute()
    {
        ...
        if (cozyWeather == null)
        {
            Debug.LogWarning("GetCozyMinute: CozyWeather instance is null.");
            storeValue.Value = 0f;
            return;
        }
        ...
        if (timeModule == null)
        {
            Debug.LogWarning("GetCozyMinute: TimeModule is null.");
            storeValue.Value = 0f;
            return;
        }

        storeValue.Value = timeModule.currentTime.minutes;
    }
}
