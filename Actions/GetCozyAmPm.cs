/*
 * Summary:
 * This script gets the AM/PM indicator from COZY: Stylized Weather 3 and stores it in a PlayMaker variable.
 *
 * License:
...
[HutongGames.PlayMaker.Tooltip("Gets the AM/PM indicator from COZY: Stylized Weather 3")]
public class GetCozyAmPm : FsmStateAction
{
    [RequiredField]
    [UIHint(UIHint.Variable)]
    [HutongGames.PlayMaker.Tooltip("Store the AM/PM string ('AM' or 'PM')")]
    ...
    void DoGetAmPm()
    {
        ...
        if (cozyWeather == null)
        {
            Debug.LogWarning("GetCozyAmPm: CozyWeather instance is null.");
            storeValue.Value = "";
            return;
        }
        ...
        if (timeModule == null)
        {
            Debug.LogWarning("GetCozyAmPm: TimeModule is null.");
            storeValue.Value = "";
            return;
        }

        int hours = timeModule.currentTime.hours;
        storeValue.Value = hours < 12 ? "AM" : "PM";
    }
}
